...
@onready var _settings_btn = %SettingsButton
@onready var _profile_btn = %ProfileButton
...
func _ready() -> void:
	...
	_settings_btn.pressed.connect(Callable(self, "_on_btn_pressed").bind(_settings_btn, "settings"))
	_profile_btn.pressed.connect(Callable(self, "_on_btn_pressed").bind(_profile_btn, "profile"))
...
func set_initial_selected_button(btn: Button) -> void:
	...
	var center = get_btn_center_top_global_position(current_btn_selected)
	_selected_indicator.set_global_position_centered(center)


func get_btn_center_top_global_position(btn: Button) -> Vector2:
	return btn.global_position + Vector2(btn.size.x * 0.5, 0) # 定位在按钮上方中心


func tween_selected_indicator(to_btn: Button) -> void:
	var target_pos = get_btn_center_top_global_position(to_btn)
