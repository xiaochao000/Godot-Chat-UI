...
func seed_app_data() -> AppData:
	...
	# 仅在没有任何配置时才添加默认 AI 配置
	if app_data.ai_configs.is_empty():
		var default_ai = AIConfig.new("default", "DeepSeek", "sk-...", "https://api.deepseek.com/v1")
		app_data.ai_configs.append(default_ai)
...
func _init_model_configs(app_data: AppData):
	# 定义所有支持的模型
	var models_data = [
		["DeepSeek-V3", "DeepSeek-V3", "deepseek-chat", "https://api.deepseek.com/v1"],
		["DeepSeek-R1", "DeepSeek-R1", "deepseek-reasoner", "https://api.deepseek.com/v1"],
		["GPT-4o", "GPT-4o", "gpt-4o", "https://api.openai.com/v1"],
		["GPT-4-Turbo", "GPT-4-Turbo", "gpt-4-turbo", "https://api.openai.com/v1"],
		["GPT-3.5-Turbo", "GPT-3.5-Turbo", "gpt-3.5-turbo", "https://api.openai.com/v1"],
		["Claude-3.5-Sonnet", "Claude-3.5-Sonnet", "claude-3-5-sonnet-20241022", "https://api.anthropic.com/v1"],
	]
	
	for model_data in models_data:
		var exists = false
		for m in app_data.model_configs:
			if m.model_name == model_data[0]:
				exists = true
				break
		
		# 只有当模型不存在时才添加
		if not exists:
			var model = ModelConfig.new(model_data[0], model_data[1], model_data[2], model_data[3])
			app_data.model_configs.append(model)
	
	# 智能迁移旧的 AIConfig 到对应的模型
	for old_config in app_data.ai_configs:
		# 跳过未激活或看起来像占位符的配置
		if not old_config.is_active or "sk-..." in old_config.api_key or old_config.api_key.length() < 10:
			continue
		
		var provider_lower = old_config.provider_name.to_lower()
		var target_models = []
		
		# 根据提供商名称匹配模型
		if provider_lower.contains("deepseek"):
			target_models = ["DeepSeek-V3", "DeepSeek-R1"]
		elif provider_lower.contains("openai") or provider_lower.contains("gpt"):
			target_models = ["GPT-4o", "GPT-4-Turbo", "GPT-3.5-Turbo"]
		elif provider_lower.contains("claude"):
			target_models = ["Claude-3.5-Sonnet"]
		
		# 为匹配的模型添加 API
		for model in app_data.model_configs:
			if model.model_name in target_models:
				# 检查是否已经存在相同的API Key，避免重复
				var api_exists = false
				for existing_api in model.api_endpoints:
					if existing_api.api_key == old_config.api_key:
						api_exists = true
						break
				
				if not api_exists:
					var api = APIEndpoint.new(
						old_config.id,
						old_config.provider_name + " (已导入)",
						old_config.api_key,
						old_config.base_url
					)
					api.status = 2 if old_config.last_test_success else 0
					model.api_endpoints.append(api)
					
					# 如果这是第一个API，设为默认选中
					if model.selected_api_index == -1:
						model.selected_api_index = 0
					
					print("已迁移 API: ", old_config.provider_name, " -> ", model.model_name)
